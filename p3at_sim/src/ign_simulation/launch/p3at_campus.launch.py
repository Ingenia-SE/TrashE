...
def generate_launch_description():
    ...
    ign_gazebo = IncludeLaunchDescription(
      ...
      #launch_arguments={'ign_args': os.path.join(pkg, 'worlds', 'campus_world.sdf')}.items(),
     )

    spawn = Node(package='ros_ign_gazebo', executable='create',
                arguments=[
                    '-name', 'p3at_ugv',
                    '-file',  os.path.join(pkg, 'models', 'p3at', 'model.sdf')
                    ],
                output='screen',
                )
    ...
    bridge = Node(
      ...
      '/model/p3at_ugv/odometry@nav_msgs/msg/Odometry[ignition.msgs.Odometry',
      '/model/p3at_ugv/pose@geometry_msgs/msg/Pose[ignition.msgs.Pose',
      '/model/p3at_ugv/tf@tf2_msgs/msg/TFMessage[ignition.msgs.Pose_V',
    ...
    return LaunchDescription([
      DeclareLaunchArgument(
          'ign_args',
          default_value=[os.path.join(pkg, 'worlds', 'campus_world.sdf')]),
        ign_gazebo,
        spawn,
        bridge,
    ])
